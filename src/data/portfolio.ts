...
export const projects: Project[] = [
  {
    ...
    architecture: [
      { label: "SEC 10-K/10-Q filings + XBRL", detail: "Real filings, indexed at ingest time — XBRL tags give exact figures instead of an LLM reading a rendered table." },
      { label: "Hybrid retrieval (BM25 + dense + reranker)", detail: "Keyword + embedding search, re-scored by an ONNX int8 cross-encoder for precision." },
      { label: "LangGraph adaptive agent routing", detail: "Simple lookups stay cheap; only multi-hop questions pay for full agentic reasoning." },
      { label: "Self-RAG faithfulness gate", detail: "Checks every claim against retrieved evidence — blocks the answer if it isn't grounded." },
      { label: "Cited answer + Next.js dashboard", detail: "Every figure links back to its filing, page and item number." },
    ],
    limitations: [
  ...
  },
  {
    ...
    architecture: [
      { label: "Plain-English deal question", detail: "e.g. \"2BR apartment in Dubai Marina under 2.2M\" — no forms, no filters to configure first." },
      { label: "Query classification agent", detail: "Routes the question to the right downstream pipeline stage." },
      { label: "Comps search + valuation agents", detail: "Real comparable transactions plus a statistical valuation model, not a single guessed number." },
      { label: "Compliance RAG (RERA corpus)", detail: "Regulatory clauses retrieved and cited, or an honest \"unable to verify\" — never a guess." },
      { label: "Memo generation + live WebSocket trace", detail: "The agent's reasoning streams live as it happens, not just a final answer." },
    ],
    limitations: [
  ...
  },
  {
    ...
    architecture: [
      { label: "Flagged transaction", detail: "A transaction pre-screened as potentially suspicious enters the pipeline." },
      { label: "Evidence + sanctions/PEP screening agents", detail: "Queries transaction networks and KYC profiles; matches against live OFAC/UN sanctions and PEP lists." },
      { label: "From-scratch GNN risk scoring", detail: "A graph convolutional network written in NumPy — no framework black box — scores accounts on network structure." },
      { label: "Typology match + regulatory RAG", detail: "Classifies against 28 SAML-D suspicious-activity typologies, grounded in a regulatory-policy corpus." },
      { label: "Verified narrative → human approval gate", detail: "A Verifier agent re-checks every citation before a human ever sees the draft — and only a human can clear the case." },
    ],
    limitations: [
  ...
  },
  {
    ...
    demoVideo: "/projects/autovaluate/loop.mp4",
    ...
    architecture: [
      { label: "Uploaded car photos (on-device)", detail: "Photos never leave the browser — inference runs entirely client-side." },
      { label: "YOLOv8/ONNX damage detection", detail: "0.732 mAP@0.5 across 18k training images, exported to ONNX for in-browser inference." },
      { label: "XGBoost quantile regression + SHAP", detail: "A price range, not a single number — with SHAP attribution showing exactly which factors moved it." },
      { label: "Hybrid RAG over real listings", detail: "Embeddings + BM25 over real Dubizzle listings, so every comparable cited is a real, checkable listing." },
      { label: "Verified valuation report (PDF/PWA)", detail: "Every number in the final report traces to an actual computed value, not a generated guess." },
    ],
    limitations: [
  ...
  },
  {
    ...
    architecture: [
      { label: "Stochastic patient arrivals", detail: "A simulator models ED patients arriving at random with varying urgency." },
      { label: "Q-Learning allocation policy", detail: "A reinforcement-learning policy learns to allocate beds, staff and equipment." },
      { label: "Constraint-based baseline", detail: "A rule-based optimiser benchmarks whether the learned policy actually earns its complexity." },
      { label: "Fairness vs. efficiency analysis", detail: "Reports Jain's fairness index (0.859) alongside utilisation (67%) — not just one number." },
      { label: "Streamlit dashboard", detail: "A public, reproducible dashboard plus a shared Colab notebook — anyone can re-run the exact experiment." },
    ],
    limitations: [
  ...
  },
  {
    ...
    architecture: [
      { label: "Transaction sequence", detail: "Fraud hides in the sequence of a customer's transactions, not any single one." },
      { label: "LSTM/RNN scoring model", detail: "Trained to output a calibrated fraud-risk score per sequence." },
      { label: "FastAPI scoring service", detail: "Serves the model behind a real API, not just a notebook." },
      { label: "Risk score + recommended action", detail: "Every score is translated into a concrete recommended business action." },
      { label: "React analyst dashboard", detail: "Sequence-analyzer and explainability views so analysts can trust each alert." },
    ],
    limitations: [
  ...
  },
  {
    ...
    architecture: [
      { label: "Store POS / sales streams", detail: "Live sales data streamed in from stores across regions." },
      { label: "Redis hot-path cache", detail: "Serves sub-second reads for live dashboards." },
      { label: "PostgreSQL system of record", detail: "Stays the durable source of truth behind the fast cache." },
      { label: "FastAPI analytics service", detail: "Layers AI-powered insights and anomaly detection on top of the stream." },
      { label: "Next.js dashboard (RBAC)", detail: "Role-based access control governs who sees which stores' sensitive financial data." },
    ],
    limitations: [
